<template>
  <firstAidNavigation :nav-text="'初始化标签'" @return="Return" @toLabelRecords="getToRecords" :isReturn="true" :isShowLabelInfo="true" />
  <view class="main">
    ...
    <view class="body">
      <view class="content margin-bottom16">
        <uni-forms :modelValue="formData" label-position="top" :rules="rules" ref="form">
          ...
          <uni-forms-item label="应有数量" style="margin-bottom: 35upx">
...
<!--      <view class="header" v-if="!overCheckingState && isChecking">-->
...
import {addMaterial, getMedicalByCode, getMedicalEncode} from "../../api/materialInfoRegistration";
import {
  endMachine,
  initMachine,
  stopMachine,
  writeMachine,
  setPower, readMachine
} from "../../util/machine";
...
const getToRecords = () => {
  uni.navigateTo({
    url: `/pages/initLabelRecords/initLabelRecords?code=${medicalTrolleyCode.value}&id=${id.value}`
  });
}
...
const initLabel = () => {
  form.value.validate().then((value) => {
    ...
    // getMedicalByCode(restoreString(res.data)).then(res => {
    //   stopMachine()
    //   endMachine(false)
...
const onSave = () => {
    ...
    // 获取编码
    getMedicalEncode({...formData.value, medicalTrolleyId: id.value}).then(res => {
      if (res.data.code === 200) {
        initMachine()
        setPower(500, 500)
        // 改写编码
        const result = writeMachine(padString(res.data.data))
        if (result.code === "200") {
          addMaterial({
            code: res.data.data,
            ...
            if (res3.data.code === 200) {
              ...
              endMachine(false)
              // 数量增加
              getMedicalByCode(restoreString(res.data.data)).then(res2 => {
                if (res2.data.code === 200) {
                  detailInfo.value = {
                    specification: res2.data.data.specification,
                    code: restoreString(res.data.data),
                    name: res2.data.data.name
                  }
                  formData.value.code = restoreString(res.data.data)
                  showDetailInfo.value = true
                }
              })
              initData()
            } else {
              errorLoad(res3.data.message)
              endMachine(false)
            }
          })
        } else {
...
function handelSave(event){
	console.log(event);
	if(event.keyCode===134 || event.keyCode === 24 ){
    initLabel()
	}
}
...
onShow(()=>{
	plus.key.addEventListener('keyup',handelSave)
	tagScanAgent = new TagScanAgent(store.state.tagUtil)
	tagScanAgent.setKeyUpListener(initLabel)
	tagScanAgent.init(500)
	// tagScanAgent.setCallBackFn(onScan)
})
onHide(()=>{
	plus.key.removeEventListener('keyup',handelSave)
