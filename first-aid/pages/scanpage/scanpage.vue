<script setup>
  ...
  // 开始扫描
	...
	const handleScan = async (data) => {
		...
		if (!code) {
			remind("未扫描到急救车标签,请确保10厘米内，无其他标签干扰")
			hideAllLoading()
			return
		} else {
			try{
...
onHide(()=>{
  ...
  const checkInfo = (data) => {
	  let rs =  toFindCode(data.data,10)
	   if(rs){//查看急救车信息
		   handleScan(data)
	   }else{
		   let rs1 = toFindCode(data.data,20)
		   if(rs1){
			   handleScanMaterial(data)
		   }else{
