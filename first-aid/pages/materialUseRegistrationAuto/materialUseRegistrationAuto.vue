<template>
  ...
  <view class="main">
    ...
    <view class="body">
      <view class="content margin-bottom16">
		...
		<view v-for="(item, index) in arrData" :key="index">  
			<MaterialInfoCardBox btnText="撤销使用" :views="item" />  
		</view> 
    ...
    <view class="footer">
      <view class="header" v-if="!showTips">
        <view :style="{color: '#ff000090', fontSize: '72upx'}">
          {{scanStateText}}
        </view>
      </view>
      <uni-load-more v-if="!showTips" status="loading" :content-text="contentText" :iconSize="36" iconType="snow"/>
    </view>
  </view>
...
watch(formData, (newValue, oldValue) => {  
  // 当 formData 实际上发生变化时添加入arrData
  if (newValue !== oldValue) {
    arrData.value.unshift(viewsData.value); // 将新的 viewsData 添加到数组的第一个位置
	noData.value = false
  }  
}, { deep: true });
  ...
  form.value.validate().then((value) => {
    addConsume({
      ...
      if (res.data.code === 200) {
        ...
        checkedArr.value.push(formData.value.code)
