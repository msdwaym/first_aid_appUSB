...
/* 传递函数 */
const emit = defineEmits(["return", "submit", "scan", "toLabelRecords", "connect", "disConnect"]); /* 点击按钮函数 */
</script>
...
<template>
  ...
  <view :style="{'background-color': props.bgColor}" class="nav">
    ...
    <view v-show="props.isShowConnection && props.isConnected" class="nav-submit" @click="emit('disConnect')">
      <image src="@/static/Nav/connected.png" style="width: 48upx; height: 42upx; margin-left: -25upx"></image>
    </view>

    <view v-show="props.isShowConnection && !props.isConnected" class="nav-submit" @click="emit('connect')">
      <image src="@/static/Nav/disConnected.png" style="width: 48upx; height: 42upx; margin-left: -25upx"></image>
    </view>
