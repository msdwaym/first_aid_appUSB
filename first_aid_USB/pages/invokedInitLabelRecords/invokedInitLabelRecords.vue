    ...
    <view class="tabs">
      <view class="tableBoxCss">
        <uni-table border emptyText="暂无更多数据" stripe style="font-size: 10rpx;">
          ...
          <!-- 表格数据行 -->
          <uni-tr v-for="record in invokedRecords" :key="record.id" @click="showInfo(record)">
...
import {deleteCode, getAdditionRecordByPage, modifyUnInvoked} from "../../api/records";
    ...
    if (res.data.code === 200) {
      ...
      getAdditionRecordByPage({
        ...
        if (res2.data.code === 200) {
          ...
          invokedRecords.value = res2.data.data.result
        } else {
          errorLoad('加载数据失败')
        }
        uni.hideLoading();
...
const rollback = async (detailInfo) => {
  showLoading('操作中...')
  const res = await modifyUnInvoked({id: detailInfo.medicalDetailId})
  if (res.data.code === 200) {
    showLoading('操作成功')
