<template>
  <firstAidNavigation :nav-text="'物资登记'" @return="backToHome" @toLabelRecords="getToRecords" :isReturn="true" :isShowLabelInfo="false" />
  <view class="main">
...
function wrightIn(event) {
  ...
  form.value.validate().then((value) => {
    if(event){
      const res = event
      if (res.code === "200" && !(res.data === "CMD_NO_TAG_ERROR") && !(res.data.length === 0)) {
        const scannedCode = restoreString(res.data[0]);
